...
public class AlbumRepositoryTest {
	...
	public void getCouple_Asterix_UderzoGoscinny() {
		Query q = em.getEntityManager().createQuery("SELECT NEW cfranc.com.comics.collector.ws.dto.AlbumShortDTO(" + 
				...
				" FROM Album a" +
				" LEFT JOIN a.serie s" +
				" LEFT JOIN a.albumpersonne ap" +
				" WHERE ap.personne.nomUsuel = :scenariste OR ap.personne.nomUsuel = :dessinateur");
		q.setParameter("scenariste", "Uderzo");
		q.setParameter("dessinateur", "Goscinny");
		List<Album> r = q.getResultList();
		int expected = 7;
		int actual = r.size();
		assertEquals(expected, actual);
	...
	public void nbAlbums_All_39() {
		Query q = em.getEntityManager().createQuery("SELECT NEW cfranc.com.comics.collector.ws.dto.AlbumCountAuthorEditorDTO("
				+ "count(a), a.editeur, ap.personne)" + 
				" FROM Album a" +
